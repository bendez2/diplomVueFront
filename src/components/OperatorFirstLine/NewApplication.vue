          ...
          <v-window v-model="tab">
            <v-window-item value="one">
              <v-row style="padding-top: 15px">
                <v-col>
                  <v-autocomplete v-model="newApplication.initiator_id" :items="initiatorsApp" item-value="id"
              ...
              <v-textarea name="input-7-1" variant="filled" label="История исполнителей" auto-grow
            ...
            <v-window-item value="two">
              <v-col>
                <v-row>
                  <v-col>
                    <v-autocomplete learable v-model="nameTypeApp" label="Тип" item-title="name" item-value="id"
                      :items="nameApps" variant="outlined"></v-autocomplete>
                  </v-col>
                  <v-col>
                    <v-autocomplete learable v-model="priorityTypeApp" label="Приоритет" item-title="name" item-value="id"
                      :items="priorityApps" variant="outlined"></v-autocomplete>
                  </v-col>
                </v-row>
  ...
  methods: {
    ...
    getInititorsApp() {
      API.get(this.api + "api/auth/users/initiators")
        .then(response => (this.initiatorsApp = response.data.data, console.log(this.initiatorsApp)));
