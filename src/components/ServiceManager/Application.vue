<template>
    <div>
        ...
        <v-card color="" style="margin: 15px" elevation="10">
            <v-col>
                <p bg-color="blue" style="font-size: 24px; margin: 20px;">
                    Просмотр заявки №{{ application.id }} от
                ...
                <v-card-text>
                    <v-window v-model="tab">
                        <v-window-item value="one">
                            <v-row style="padding-top: 15px">
                                <v-col>
                                    <v-autocomplete v-model="application.initiator_id" :items="initiatorsApp"
                                        item-value="id" item-title="name" label="Инициатор" variant="outlined"
                                        readonly></v-autocomplete>
                                    <v-text-field readonly v-model="application.location" label="Место оказания услуги"
                            ...
                            <v-textarea readonly name="input-7-1" variant="filled" label="Текст заявки" auto-grow
